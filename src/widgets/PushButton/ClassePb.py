...
class ClassePb:
    ...
    def ENT_CHECK(self, event):
        if self.wg.isEnabled():
            if not self.wg.isChecked():
                Functions().SET_ICON_A_SUPPR(wg=self.wg, img=self.img_uncheck_hover + self.img_uncheck_hover_rgb, dim=self.dim_ico)
            else:
                Functions().SET_ICON_A_SUPPR(wg=self.wg, img=self.img_check_hover + self.img_check_hover_rgb, dim=self.dim_ico)
    def LVE_CHECK(self, event):
        if self.wg.isEnabled():
            if not self.wg.isChecked():
                Functions().SET_ICON_A_SUPPR(wg=self.wg, img=self.img_uncheck + self.img_uncheck_rgb, dim=self.dim_ico)
            else:
                Functions().SET_ICON_A_SUPPR(wg=self.wg, img=self.img_check + self.img_check_rgb, dim=self.dim_ico)
    ...
    def ENT_ICO(self, event):
        if not self.wg.isChecked() and self.wg.isEnabled():
            Functions().SET_ICON_A_SUPPR(wg=self.wg, img=self.img + self.img_hover_rgb, dim=self.dim_ico)
    def LVE_ICO(self, event):
        if not self.wg.isChecked() and self.wg.isEnabled():
            Functions().SET_ICON_A_SUPPR(wg=self.wg, img=self.img + self.img_rgb, dim=self.dim_ico)
